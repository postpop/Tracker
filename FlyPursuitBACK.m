classdef FlyPursuit < handle
   ...
   methods (Access='public')
      ...
      function trackNextFrame(obj)
         ...
         [bw, obj.foreGround, clusterGroup, Nconn] = obj.getForeGroundAdaptive(oriFrame, obj.foreGroundOLD, obj.gmmStart.mu, obj.foreGroundThreshold);
         %%  cluster
         gmmNew = obj.clusterComponents(bw, gmm, frame, clusterGroup, Nconn);
         % save initial positions for next frame
         obj.mu(f,:,:) = gmmNew.mu;
         obj.sigma(f,:,:,:) = gmmNew.Sigma;
         %plot(gmmNew.mu(:,1),gmmNew.mu(:,2),'xk')
         % get each fly's orientation and area from eig analysis of their xcov
         for i = 1:obj.nFlies
            [eigVec, eigVal] = eig(gmmNew.Sigma(:,:,i));
            eigVal = diag(eigVal);
            obj.orientation(f,i,:) = eigVec(:, argmax(eigVal));
            obj.area(f,i) = pi*prod(sqrt(eigVal));
         end
         ...
         obj.pathLabels(f,:) = newLabels;% labels for each obj.centroid
         obj.tracks(f,obj.pathLabels(f,:),:) = squeeze(obj.mu(f,:,:));% actual obj.tracks
      end
