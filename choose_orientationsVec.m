...
% the velocity term. 
% vThres (by default = 0): minimal velocity

% 130821 - added hard threshold to velocity vector and squared v, JC

function theta = choose_orientationsVec(x,y,theta,velocity_angle_weight,max_velocity_angle_weight, vThres)
...
stateprev = zeros(N-1,2,nFlies);

% allocate space for computing costs
tmpcost = zeros(2,nFlies);
costprevnew = zeros(2,nFlies);

% initialize first frame
costprev = zeros(2,nFlies);

% compute velocity
vx = [zeros(1,nFlies);diff(x)];
vy = [zeros(1,nFlies);diff(y)];
v = sqrt(vx.^2+vy.^2);
% threshold v
%v(v<vThres) = 0; 
% smooth v
%v = conv(v, normalizeSum(gausswin(16,3)),'same');
